    ...
    <div className="w-full overflow-x-auto rounded-2xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/90 shadow-sm dark:shadow-xl transition-colors">
      <table className="w-full text-left text-xs sm:text-sm">
        <thead className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 uppercase text-[11px] font-bold tracking-wider">
          <tr>
            <th className="px-4 py-3.5">{t.athlete}</th>
            <th className="px-4 py-3.5">{t.belts}</th>
            <th className="px-4 py-3.5">{t.gender} / {t.age}</th>
            <th className="px-4 py-3.5">{t.weight} (Kg)</th>
            <th className="px-4 py-3.5">{t.wtDivision}</th>
            <th className="px-4 py-3.5">{t.representingClub}</th>
            ...
            return (
              <tr key={player.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
                ...
                <td className="px-4 py-3.5">
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">{t.dob}: {player.date_of_birth}</div>
                </td>
                ...
                <td className="px-4 py-3.5">
                  <div className="flex items-center gap-1.5 font-bold">
                    ...
                    />
                    <span>{lang === 'my' ? (player.gender === 'Male' ? 'ကျား' : player.gender === 'Female' ? 'မ' : player.gender) : player.gender}</span>
                  </div>
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">{age} {t.years} ({division.category})</div>
                </td>
